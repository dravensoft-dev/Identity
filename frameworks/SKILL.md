...
| Component | What it is | Takes | Behaviour | Layers |
|---|---|---|---|---|
| `ArenaBarChart` | Categorical bars on one axis. Dependency-free SVG that reads the token layer directly, with a visually-hidden table of the same numbers. | `labels*` `values*` `seriesLabel*` `slot` `slots` `tone` `valueSuffix` `valuePrefix` `valueFormat` `height` `minPointSpacing` | figure-with-data-table | angular, react |

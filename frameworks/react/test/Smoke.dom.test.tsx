...
test('mount renders a React tree into a real document', () => {
  ...
  assert.ok(el.querySelector<HTMLElement>('[role="dialog"]') !== null);
  assert.equal(el.querySelector<HTMLElement>('[role="dialog"]')!.getAttribute('aria-modal'), 'true');
});

test('mount resolves an implicit role through a real element, not a string', () => {
  const el = mount(<button type="button">Go</button>);
  assert.equal(el.querySelector<HTMLElement>('button')!.tagName, 'BUTTON');
});

...
test('destructive paints the filled danger surface, and nothing else does', () => {
  ...
  assert.match(html, /\bbg-error-fill\b/, 'the filled danger surface does not render');
  assert.match(html, /\btext-error-content\b/, 'the filled surface carries no readable ink');

  const plain = renderToStaticMarkup(<ConfirmDialog open title="Save" onConfirm={() => {}} />);
  assert.doesNotMatch(plain, /\bbg-error-fill\b/, 'an ordinary confirm must not be filled with danger');
  assert.match(plain, /\bbg-primary\b/, 'the ordinary confirm lost its primary surface');
});

...
import { PAD } from '../../../DataVisuals.js';
...
const xOf = (i) => PAD.l + (IW / (VALUES.length - 1)) * i;

afterEach(cleanup);

const edgeAt = (left) => () => ({ left });

function stubBoxes(root) {
  const svg = root.querySelector('svg');
  const overlay = root.querySelector('rect[fill="transparent"]');
  svg.getBoundingClientRect = edgeAt(SVG_LEFT);
  overlay.getBoundingClientRect = edgeAt(SVG_LEFT + PAD.l);
  Object.defineProperty(overlay, 'ownerSVGElement', { value: svg, configurable: true });
  return overlay;
}

function crosshairX(root) {
...
function hoverAt(overlay, clientX) {
  act(() => {
...
test('the pointer and the plot share the SVG origin, so a pointer on a point snaps to that point', () => {
  const root = mount(<LineChart seriesLabel="p95" labels={LABELS} values={VALUES} />);

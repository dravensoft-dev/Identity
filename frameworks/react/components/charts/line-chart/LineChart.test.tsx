...
test('LineChart with no valueSuffix draws bare numbers', () => {
  const html = renderToStaticMarkup(<LineChart seriesLabel="p95 latency" labels={LABELS} values={VALUES} />);
...
test('LineChart draws the area fill only when area is set', () => {
  const withArea = renderToStaticMarkup(<LineChart seriesLabel="p95 latency" labels={LABELS} values={VALUES} area />);
  const without = renderToStaticMarkup(<LineChart seriesLabel="p95 latency" labels={LABELS} values={VALUES} />);
...
test('LineChart drops a consumer style object and a consumer attribute, each independently', () => {
  const html = renderToStaticMarkup(
    // @ts-expect-error the contract refuses this on purpose, and the render is what this asserts
    <LineChart seriesLabel="p95 latency" labels={LABELS} values={VALUES} style={{ color: '#ff00ff' }} data-stray="x" />
  );
...
test('LineChart drops a label with no value at its index, rather than drawing it over empty plot', () => {
  const html = renderToStaticMarkup(
    <LineChart seriesLabel="p95 latency" labels={['Alpha', 'Beta', 'SURPLUS']} values={[10, 20]} />
  );
...
test('LineChart draws an empty label for a point with no label, rather than throwing or printing undefined', () => {
  const html = renderToStaticMarkup(<LineChart seriesLabel="p95 latency" labels={['Only']} values={[10, 20]} />);

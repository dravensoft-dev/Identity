...
test('ArenaLineChart with no valueSuffix draws bare numbers', () => {
  const html = renderToStaticMarkup(<ArenaLineChart seriesLabel="p95 latency" labels={LABELS} values={VALUES} />);
...
test('ArenaLineChart throws when labels is absent, which required-ness demands of every layer', () => {
  ...
  assert.throws(() => renderToStaticMarkup(<ArenaLineChart seriesLabel="p95 latency" values={VALUES} />), /ArenaLineChart: `labels` is required/);
});

test('ArenaLineChart throws when values is absent, which required-ness demands of every layer', () => {
  // @ts-expect-error the contract refuses this on purpose, and the render is what this asserts
  assert.throws(() => renderToStaticMarkup(<ArenaLineChart seriesLabel="p95 latency" labels={LABELS} />), /ArenaLineChart: `values` is required/);
});

test('ArenaLineChart draws the area fill only when area is set', () => {
  const withArea = renderToStaticMarkup(<ArenaLineChart seriesLabel="p95 latency" labels={LABELS} values={VALUES} area />);
  const without = renderToStaticMarkup(<ArenaLineChart seriesLabel="p95 latency" labels={LABELS} values={VALUES} />);
...
test('ArenaLineChart drops a consumer style object and a consumer attribute, each independently', () => {
  const html = renderToStaticMarkup(
    // @ts-expect-error the contract refuses this on purpose, and the render is what this asserts
    <ArenaLineChart seriesLabel="p95 latency" labels={LABELS} values={VALUES} style={{ color: '#ff00ff' }} data-stray="x" />
  );
...
test('ArenaLineChart drops a label with no value at its index, rather than drawing it over empty plot', () => {
  const html = renderToStaticMarkup(
    <ArenaLineChart seriesLabel="p95 latency" labels={['Alpha', 'Beta', 'SURPLUS']} values={[10, 20]} />
  );
...
test('ArenaLineChart draws an empty label for a point with no label, rather than throwing or printing undefined', () => {
  const html = renderToStaticMarkup(<ArenaLineChart seriesLabel="p95 latency" labels={['Only']} values={[10, 20]} />);

 ...
 * overlay rect's, which differ by ARENA_PAD.l exactly as they do in a browser. The series is
...
const IW = WIDTH - ARENA_PAD.l - ARENA_PAD.r;
const xOf = (i: number) => ARENA_PAD.l + (IW / (VALUES.length - 1)) * i;
...
function stubBoxes(root: ParentNode) {
  ...
  overlay!.getBoundingClientRect = edgeAt(SVG_LEFT + ARENA_PAD.l) as () => DOMRect;
...
test('the pointer and the plot share the SVG origin, so a pointer on a point snaps to that point', () => {
  ...
  for (let i = 0; i < VALUES.length; i++) {
    ...
    assert.equal(crosshairX(root), xOf(i),
      `a pointer sitting exactly on point ${i} selected another point -- the two origins differ by ARENA_PAD.l`);
  }
});

test('the interval is narrower than the left pad, so the wrong origin would be visible', () => {
  const step = IW / (VALUES.length - 1);
  assert.ok(step < ARENA_PAD.l * 2,
    `this fixture cannot see the defect: an interval of ${step}px absorbs a ${ARENA_PAD.l}px shift`);
});

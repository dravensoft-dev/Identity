...
function narrowWidths(width, body) {
  const saved = globalThis.ResizeObserver;
  globalThis.ResizeObserver = class {
    constructor(callback) { this.callback = callback; }
    observe(target) { this.callback([{ target, contentRect: { width } }]); }
    disconnect() {}
  };
  try {
    return body();
  ...
  assertPatternCases({
    ...
    cases: {
      ...
      wide: () => {
        ...
        const cellsOf = (row) => [...row.querySelectorAll('[role="gridcell"], [role="columnheader"]')];
        const cells = gridRows.map(cellsOf);

        const start = cells[0][0];
        act(() => { start.focus(); });
        assert.equal(root.querySelectorAll('[tabindex="0"]').length, 1, 'a grid is ONE tab stop');
      ...
      card: () => narrowWidths(400, () => {
        ...
        assert.equal(root.querySelector('table'), null, 'the card shape renders no table element');
        assert.equal(root.querySelectorAll('[role="gridcell"]').length, 0,

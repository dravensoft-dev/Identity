...
| `removable` | primitive | `boolean` | `false` | Whether the dismiss × is shown. Every layer gates the × on this member and never on whether anything listens for `remove`, per R6. Removability is a declared input, not something inferred from the event. |

...
test('the feed is a list, not a stack of divs', () => {
  const html = renderToStaticMarkup(<ActivityFeed label={LABEL} items={ITEMS} />);
...
test('each part of the grammar takes its own ink', () => {
  ...
  assert.match(html, /\btext-secondary\b/, 'the target reads in the secondary ink');
  assert.match(html, /\btext-base-content\/62\b/, 'the time reads muted');
});

test('tone drives the dot and defaults to accent', () => {
  assert.match(renderToStaticMarkup(<ActivityFeed label={LABEL} items={[ITEMS[0]!]} />), /\btext-primary\b/);
  assert.match(renderToStaticMarkup(<ActivityFeed label={LABEL} items={[ITEMS[1]!]} />), /\btext-error\b/);
});
...
test('an item missing a target or a time still renders', () => {
  const html = renderToStaticMarkup(<ActivityFeed label={LABEL} items={[{ id: '4', actor: 'CI', action: 'restarted' }]} />);
...
test('every field is drawn by Arena, and there is no per-item projection', () => {
  const html = renderToStaticMarkup(
    <ActivityFeed label={LABEL} items={[{ id: 'a1', actor: 'Ada', action: 'deployed', target: 'api-7', time: '2m' }]} />,
  );
...
test('an absent items array throws rather than rendering an empty feed', () => {
  // @ts-expect-error the contract refuses this on purpose, and the render is what this asserts
  assert.throws(() => renderToStaticMarkup(<ActivityFeed label={LABEL} />), /`items` is required/);
});

test('a consumer style prop and stray attributes are dropped, not spread onto the <ul>', () => {
  const html = renderToStaticMarkup(
    // @ts-expect-error the contract refuses this on purpose, and the render is what this asserts
    <ActivityFeed label={LABEL} items={[]} style={{ color: 'rgb(255, 0, 0)' }} data-escape="leaked" />,
  );
...
test('an absent label throws -- a feed a reader navigates BY cannot be nameless', () => {
  // @ts-expect-error the contract refuses this on purpose, and the render is what this asserts
  assert.throws(() => renderToStaticMarkup(<ActivityFeed items={ITEMS} />),
    /ActivityFeed: `label` is required/);
});

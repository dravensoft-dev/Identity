...
test('each variant takes its own box from the recipe, and a consumer string still overrides it', () => {
  assert.match(renderToStaticMarkup(<Skeleton variant="circle" />), /\barena-skeleton__root--variant-circle\b/);
  assert.match(renderToStaticMarkup(<Skeleton variant="line" />), /\b(?:arena-skeleton__line|arena-skeleton__last-line|arena-skeleton__root--variant-line)\b/);
  assert.match(renderToStaticMarkup(<Skeleton variant="block" />), /\barena-skeleton__root--variant-block\b/);
});

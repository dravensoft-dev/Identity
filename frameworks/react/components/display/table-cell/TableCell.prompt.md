...
**Do / Don't**
- Put a value in it, or one of Arena's own components — a `Badge` for a status, a `Button` for an action. This member is why `Table` is a compound component at all: a column's `render` function was per-item projection, which the library does not do, but a cell **you** instantiate is just an element you wrote.

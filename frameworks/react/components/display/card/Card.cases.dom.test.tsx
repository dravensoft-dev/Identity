/* An interactive Card is a role="button" div rather than a <button> element, and that is not a
 * shortcut: a card body is where a consumer puts their own controls, and a control nested inside
 * a control is reachable by nobody. TableRow's card shape made the same call for the same reason.
 * The surface case passes no slots, because content a consumer projects is theirs: a Button in
...
const BINDING = join(REACT_COMPONENTS, 'display/card/Card.behaviour.json');
...
test('Card meets both of its declared shapes', () => {
  assertPatternCases({
    bindingPath: BINDING,
    cases: {
      ...
      interactive: () => {
        let clicked = 0;
        const root = mount(<Card title="checkout-api" interactive onClick={() => { clicked += 1; }} />);
        const el = root.firstElementChild;
        ...
        let blocked = 0;
        const off = mount(<Card title="checkout-api" interactive disabled onClick={() => { blocked += 1; }} />);
...
test('a keypress on a control inside the card does not activate the card as well', () => {
  let clicked = 0;
  const root = mount(
    <Card title="checkout-api" interactive onClick={() => { clicked += 1; }}>

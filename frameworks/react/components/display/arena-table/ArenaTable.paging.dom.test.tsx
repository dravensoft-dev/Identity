...
test('with no `page` no pager is drawn, which is every table that fits', () => {
  const root = mount(<ArenaTable label={LABEL} columns={COLUMNS} responsive={false}>{rows(2)}</ArenaTable>);
  assert.equal(root.querySelector('nav'), null);
});

test('a pager is drawn, and it is named from the table rather than from a constant', () => {
  const root = mount(
    <ArenaTable label={LABEL} columns={COLUMNS} responsive={false} page={{ index: 1, size: 20, total: 96 }}>
      {rows(2)}
    </ArenaTable>,
  );
...
test('an empty table draws no pager, because there is no grid for it to sit under', () => {
  const root = mount(
    <ArenaTable label={LABEL} columns={COLUMNS} responsive={false} page={{ index: 1, size: 20, total: 0 }} />,
  );
  assert.equal(root.querySelector('nav'), null);

...
const BINDING = join(REACT_COMPONENTS, 'navigation/command-palette/CommandPalette.behaviour.json');
...
function render(extra) {
  const root = mount(<CommandPalette open commands={COMMANDS} onClose={() => {}} onRun={() => {}} {...extra} />);
  return { root, input: root.querySelector('[role="combobox"]'), options: [...root.querySelectorAll('[role="option"]')] };
}
...
test('a query matching nothing leaves no dangling active-descendant behind', () => {
  ...
  assert.equal(root.querySelectorAll('[role="option"]').length, 0, 'the fixture must actually filter to nothing');
...
test('Escape and Enter both report through the palette own channels', () => {
  ...
  assert.equal(ran?.id, 'deploy', 'Enter did not run the active command');

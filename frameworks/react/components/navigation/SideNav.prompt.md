...
```jsx
<SideNav ariaLabel="Primary" active={route}
  onNav={(id, event) => { event.preventDefault(); setRoute(id); }}
  items={[
...
- **Do** give every destination an `href`, even in a single-page app. It is what lets
  the item be opened in a new tab and announced as a link. `onNav` receives the click
  event as its second argument for exactly this case: a single-page app calls
  `event.preventDefault()` and routes itself, so the anchor stays a real link without

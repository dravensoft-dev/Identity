/* Like RadioGroup, this control is native radios in disguise: each segment wraps
...
const BINDING = join(REACT_COMPONENTS, 'navigation/segmented-control/SegmentedControl.behaviour.json');
...
test('SegmentedControl is a named radiogroup over native radios', () => {
  const root = mount(
    <SegmentedControl ariaLabel="Time range" name="range" options={OPTIONS} value="7d" onChange={() => {}} />,
  );
  ...
  assert.equal(radios.filter((r) => r.checked).length, 1, 'exactly one option may be checked');
  assert.equal(radios[1].checked, true, 'the checked option must be the one matching `value`');

  assert.equal(group.querySelectorAll('[tabindex]').length, 0,
...
test('a segment reports its own value, not a DOM event', () => {
  let picked = null;
  const root = mount(
    <SegmentedControl ariaLabel="Time range" options={OPTIONS} value="7d" onChange={(v) => { picked = v; }} />,
  );
  const radios = [...root.querySelectorAll('input[type="radio"]')];
  radios[2].click();

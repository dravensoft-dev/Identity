...
test('a consumer className does not reach the input -- its class is the manifest\'s and nothing else', () => {
  ...
  assert.match(html, /<input[^>]*class="[^"]*\bbg-transparent\b[^"]*"/);
...
test('the picker indicator is styled by the manifest, so nothing injects a stylesheet for it', () => {
  ...
  assert.match(html, /webkit-calendar-picker-indicator\]:cursor-pointer/,
...
test('the three field states are three branches of one recipe', () => {
  ...
  assert.match(neutral, /focus-within:border-secondary/, 'focus is a modifier, not a reported state');
  assert.match(neutral, /\bborder-base-300\b/);

  const bad = renderToStaticMarkup(<Input label="A" error="Nope" />);
  assert.match(bad, /\bborder-error\b/);
  assert.match(bad, /\btext-error\b/);

  const good = renderToStaticMarkup(<Input label="A" valid />);
  assert.match(good, /\bborder-success\b/);
  assert.match(good, /\btext-success\b/);
});

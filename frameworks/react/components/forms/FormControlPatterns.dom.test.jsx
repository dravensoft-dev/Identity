...
function assertKeysUnintercepted(el, keys) {
...
test('Button reflects disabled natively, and the reflection tracks the member', () => {
  const off = mount(<Button disabled onClick={() => {}}>Deploy</Button>).querySelector('button');
  assert.equal(off.hasAttribute('disabled'), true, 'a disabled button must be disabled');
  cleanup();
  const on = mount(<Button onClick={() => {}}>Deploy</Button>).querySelector('button');
...
test('Checkbox is a native checkbox whose checked state is the platform’s', () => {
  ...
  assert.equal(el.checked, true, 'the fixture must render the checked state it claims');
  assertKeysUnintercepted(el, [' ']);

  const off = mount(<Checkbox label="Notify the team" onChange={() => {}} />)
    .querySelector('input[type="checkbox"]');
  assert.equal(off.checked, false, 'the state must track the member rather than being hardcoded');
...
test('Select is a native select: the browser owns the popup, the author owns the name', () => {
  ...
  const label = root.querySelector(`label[for="${el.getAttribute('id')}"]`);

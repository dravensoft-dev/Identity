...
const fallback = () => config.default ?? config.palettes[0]?.name ?? "dark";
const declares = (name) => name !== null && names().includes(name);
...
export function toggleArenaTheme() {
  ...
  if (next === undefined)
    throw new Error("toggleArenaTheme: no palette is declared");

...
import { HEADER_LABEL, CELL_BASE } from "../table-cell/TableCell.generated.js";
export function Table({ columns, children, empty = "No data.", responsive = true, label }) {
  ...
  const cellCounts = rowEls.map((row) => React.isValidElement(row) ? React.Children.toArray(row.props.children).length : 0);
  ...
  const onGridKeyDown = (e) => {
    ...
    if (e.key === "ArrowUp")
      ...
      if (rowEl && React.isValidElement(rowEl) && rowEl.props.onClick) {
        rowEl.props.onClick();
      }
      return;
    } else
  ...
  const headerNav = (ci) => ({
    tabIndex: curRow === 0 && ci === curCol ? 0 : -1,
    onFocus: (e) => {
  ...
  }, rowEls.length === 0 && React.createElement("div", {
    style: {
  ...
  }, empty), rowEls.map((row, ri) => React.isValidElement(row) ? React.cloneElement(row, { rowIndex: ri + 1, columns, layout: "card" }) : row)) : React.createElement("div", {
    style: {
      border: "var(--bw) solid var(--color-base-300)",
      borderRadius: "var(--r-lg)",
      overflow: "hidden",
  ...
  }, c.header)))), React.createElement("tbody", null, rowEls.length === 0 && React.createElement("tr", {
    role: "row"
  ...
  }, empty)), rowEls.map((row, ri) => React.isValidElement(row) ? React.cloneElement(row, {
    rowIndex: ri + 1,
    columns,
    layout: "table",

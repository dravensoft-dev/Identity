...
import { useContainerWidth, readBreakpoint } from "../../../UseContainerWidth.generated.js";
import { catColor } from "../../../DataVisuals.generated.js";
...
import {
  ...
  weekdayOf,
  formatDate
} from "./CalendarInternals.generated.js";
...
export function Calendar({
  ...
  const kids = useMemo(() => React.Children.toArray(children).filter((c) => React.isValidElement(c)), [children]);
  ...
  for (let m = Math.ceil(startMin / 60) * 60;m <= endMin; m += 60)
    hours.push(m);
  const slots = hours.map((m, i) => ({ start: m, end: (i + 1 < hours.length ? hours[i + 1] : endMin) ?? endMin })).filter((s) => s.end > s.start);
  const slotFor = (cols) => width === null ? null : (width - calendarGutterW) / days.length / cols;
  ...
  const activateDay = (iso) => dayInteractive ? () => {
    onDateClick && onDateClick(iso);
  } : undefined;
  ...
  const onGridKeyDown = (e) => {
    ...
    if (t.getAttribute("role") === "gridcell") {
      ...
      if (e.key === "ArrowLeft")
        ...
        hour = Math.min(slots.length - 1, hour + 1);
      else if (e.key === "Home")
        hour = 0;
      else if (e.key === "End")
        hour = slots.length - 1;
  ...
  return React.createElement("section", {
    ...
    return React.createElement(DayHead, {
      key: d,
    ...
    ref: gridRef,
    role: "grid",
    "aria-label": `Schedule grid, ${rangeTitle(days)}`,
    ...
    return React.createElement("div", {
      key: s.start,
      role: "gridcell",
      "aria-label": formatHM(s.start),
      tabIndex: isCursor ? 0 : -1,
      onFocus: () => {
        if (di !== curDay || si !== curHour)
          setCursor({ day: di, hour: si });
      },
      style: {
        position: "absolute",
        top: y(s.start),
        left: 0,
        right: 0,
        height: y(s.end) - y(s.start),
        outline: "none",
        boxShadow: isCursor && gridFocused ? "inset 0 0 0 var(--focus-width) var(--focus-ring)" : undefined
      }
    });
  }), (byDay[di] ?? []).map((p) => {

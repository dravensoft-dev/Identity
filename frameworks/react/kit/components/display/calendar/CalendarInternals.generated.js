...
export function showsTime(chipHeight, slotWidth) {
  if (chipHeight < calendarTimeMinH)
    return false;
...
function warnOnce(message) {
  if (warned.has(message) || typeof console === "undefined")
    return;
  warned.add(message);
...
function partsFormatter(timeZone) {
  ...
  try {
    ...
    if (timeZone === "UTC")
      throw new Error("[arena] calendar: Intl rejected UTC.");
...
const nowParts = (timeZone) => {
  ...
  if (!parts)
    throw new Error(`[arena] calendar: the current instant is unreadable in "${timeZone}".`);
  return parts;
};
export const todayIso = (timeZone) => isoDateOf(nowParts(timeZone));
export const nowMinutes = (timeZone) => minutesOf(nowParts(timeZone));
...
export function layoutDay(placements) {
  ...
  const flush = () => {
    for (const p of cluster)
      out.push({ ...p, cols: colEnds.length });
    cluster = [];
    colEnds = [];
    clusterEnd = -1 / 0;
  };
...
export function rangeTitle(days) {
  ...
  if (a === undefined || b === undefined)
    return "";
  if (a === b)
    return formatDate(a, { weekday: "short", day: "numeric", month: "short", year: "numeric" });
  const A = asUtcDate(a);
  const B = asUtcDate(b);
  const sameYear = A.getUTCFullYear() === B.getUTCFullYear();
  const sameMonth = sameYear && A.getUTCMonth() === B.getUTCMonth();
  const left = sameMonth ? formatDate(a, { day: "numeric" }) : sameYear ? formatDate(a, { day: "numeric", month: "short" }) : formatDate(a, { day: "numeric", month: "short", year: "numeric" });

...
export function Shell({ active = 'dashboard', onNav, title, actions, children }) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '232px 1fr', minHeight: '100%', background: 'var(--bg)' }}>
      <aside style={{ borderRight: 'var(--bw) solid var(--color-base-300)', padding: '24px 16px', display: 'flex', flexDirection: 'column', gap: 4 }}>
        ...
        <div style={{ marginTop: 'auto', display: 'flex', alignItems: 'center', gap: 10, padding: '12px 8px', borderTop: 'var(--bw) solid var(--color-base-300)' }}>
          <div style={{ width: 30, height: 30, borderRadius: '50%', background: 'var(--gold)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--ink)', fontFamily: 'var(--font-display)', fontWeight: 'var(--fw-extrabold)', fontSize: 'var(--dz-text-md)' }}>A</div>
      ...
      <main style={{ display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        <header style={{ padding: '20px 32px', borderBottom: 'var(--bw) solid var(--color-base-300)' }}>

...
declare global {
  interface Window { __toggleTheme?: () => void }
}

function flipTheme() {
  const toggle = typeof window !== 'undefined' ? window.__toggleTheme : undefined;
  if (toggle) {
    toggle();
    return;
  }
...
export function Shell({ active = 'dashboard', onNav, title, actions, children }: ShellProps) {

...
being ported. **What it carries is its own selector list and shrinks with every Plan D
batch, so read the CSS rather than this sentence**; what is left today is the progress
spinner and bar, and SideNav's nav list. The rest of Material's components still render with Material's
...
**Two whole families are gone, and the reason is the rule rather than a cleanup.** A
dressing block dies when no delegated entry still needs it. No Material button is dressed:
`arena-button` and `arena-icon-button` are primitives, so the `.mat-mdc-unelevated-button`,
`.mat-mdc-outlined-button` and `.arena-danger` blocks went with the last delegated entry
that cited them. **No form field is dressed either** -- `arena-input`, `arena-textarea` and
now `arena-select` are all primitives, so the outlined-field block went with `Select`'s
entry, and a `<mat-form-field>` in an adopting app wears Material's own look whatever its
appearance. The dialog surface and the snack-bar container went the same way, with
`arena-dialog` and `arena-toast`.
...
- Reach for `arena-input`, `arena-textarea` and `arena-select` rather than a
  `<mat-form-field>`. The bridge dresses no form field at all now, in either appearance, so
  a Material field in an Arena app is a field that does not wear Arena.

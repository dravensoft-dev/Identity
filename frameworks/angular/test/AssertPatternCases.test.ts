/* The wrapper's own failure paths. These write bindings that are deliberately
   ...
   None of these four needs a component render, so this file needs neither
   ensureDom() nor useTestEnvironment() -- assertPatternCases throws before
   any thunk runs, in every case below. */
import test from 'node:test';
import assert from 'node:assert/strict';
import { writeFileSync, mkdtempSync } from 'node:fs';
import { join } from 'node:path';
import { tmpdir } from 'node:os';
import { assertPatternCases } from './Compliance';

function bindingFile(binding: unknown): string {
...
test('a case name the binding does not declare is refused', () => {
  ...
  assert.throws(
    () => assertPatternCases({
      bindingPath: p,
      cases: {
        danger: () => ({ root: null as unknown as Element }),
        advisory: () => ({ root: null as unknown as Element }),
        typo: () => ({ root: null as unknown as Element }),
      },
    }),
    /typo/,
  );
...
test('a flat binding cannot be asserted through the cases entry point', () => {
  ...
  assert.throws(
    () => assertPatternCases({ bindingPath: p, cases: { only: () => ({ root: null as unknown as Element }) } }),
    /declares no cases/,
  );
});

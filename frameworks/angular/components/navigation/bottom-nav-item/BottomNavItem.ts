...
import { BottomNavState } from '../bottom-nav/BottomNavState';
import { bottomNavStyles } from '../bottom-nav/BottomNav.variants';
...
export class BottomNavItem {
  /** Identifies the destination. BottomNav.active names one of these, and the item whose id matches is the one marked aria-current="page". Required, and guarded with a falsy check rather than an absence check: a blank id can never match and is an omission wearing a value. */
  ...
  protected readonly styles = computed(() => bottomNavStyles({ active: this.on() }));

  protected readonly name = computed(() => {
    const text = this.label();
    if (text.trim() === '') {
      throw new Error('BottomNavItem: `label` is required, and is what the destination reads');
    }
    return text;
  });
  ...
  protected readonly glyphClass = computed(() => {
    ...
    if (glyph.trim() === '') {
      throw new Error('BottomNavItem: `icon` is required, and a column of a bar has no room to stand without one');
    }
  ...
  private key(): string {
    const value = this.id();
    if (value.trim() === '') {
      throw new Error('BottomNavItem: `id` is required, and is what BottomNav.active names');
    }
    return value;
  }
}

...
const BINDING = join(ANGULAR_COMPONENTS, 'navigation/tabs/Tabs.behaviour.json');
const TAB_BINDING = join(ANGULAR_COMPONENTS, 'navigation/tab/Tab.behaviour.json');
...
test('two Tabs on one page generate distinct ids, so the second does not steal the first\'s panel', () => {
  const first = render();
  const second = render();
  try {

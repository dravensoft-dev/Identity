 ...
 * an empty string and a zero page count both satisfy it and neither is a Pagination. */
...
const BINDING = join(ANGULAR_COMPONENTS, 'navigation/pagination/Pagination.behaviour.json');
...
test('an ariaLabel bound to nothing throws, because input.required only proves it was bound', () => {
  ...
  try {
    assert.throws(() => fixture.detectChanges(), /Pagination: .ariaLabel. is required/,
...
test('a pageCount below one throws, because a window over nothing is not a window', () => {
  for (const pageCount of [0, -1, 2.5]) {
    ...
    try {
      assert.throws(() => fixture.detectChanges(), /Pagination: .pageCount. is required/,

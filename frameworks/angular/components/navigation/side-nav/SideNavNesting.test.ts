...
const SECTION_BINDING = join(ANGULAR_COMPONENTS, 'navigation/side-nav-section/SideNavSection.behaviour.json');
const COLLAPSIBLE_BINDING = join(ANGULAR_COMPONENTS, 'navigation/side-nav-collapsible/SideNavCollapsible.behaviour.json');

@Component({
  standalone: true,
  imports: [SideNav, SideNavItem, SideNavSection, SideNavCollapsible],
  template: `
...
@Component({
  standalone: true,
  imports: [SideNav, SideNavItem, SideNavCollapsible],
  template: `
...
test('a section with no children throws, because its heading would name nothing', () => {
  @Component({
    standalone: true,
    imports: [SideNav, SideNavSection],

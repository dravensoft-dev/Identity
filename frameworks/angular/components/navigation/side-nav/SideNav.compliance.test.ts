...
const BINDING = join(ANGULAR_COMPONENTS, 'navigation/side-nav/SideNav.behaviour.json');
...
test('the landmark refuses a blank name at runtime, because a required input is not a guard', () => {
  @Component({ standalone: true, imports: [SideNav], template: `<arena-side-nav ariaLabel=" " />` })
  class BlankHost {}

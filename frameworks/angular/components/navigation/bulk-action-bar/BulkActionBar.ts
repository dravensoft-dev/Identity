...
export class BulkActionBar {
  ...
  readonly actions = input.required<readonly BulkAction[]>();
  ...
  readonly layout = input<BulkActionBarLayout>('auto');
  ...
  protected classesFor(action: BulkAction) {
    return bulkActionBarStyles({ destructive: !!action.destructive, narrow: this.narrow() });
  }
}

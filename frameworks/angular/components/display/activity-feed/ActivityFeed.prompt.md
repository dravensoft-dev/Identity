...
time; `tone` colours the leading dot from Badge's vocabulary.
...
| `items*` | array | `readonly ActivityItem[]` |  | The events, newest first by convention. Each row is drawn by Arena; there is no per-item projection. |

...
const BINDING = join(ANGULAR_COMPONENTS, 'display/arena-badge/ArenaBadge.behaviour.json');
...
test('`dot` takes the booleanAttribute transform, so the bare attribute form means true', () => {
  @Component({ standalone: true, imports: [ArenaBadge], template: `<arena-badge dot>Live</arena-badge>` })
  class BareAttribute {}

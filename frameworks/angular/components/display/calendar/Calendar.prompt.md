...
**A chip is not a DOM child of its day column here, and `aria-owns` is why that is invisible.** React nests each chip inside its day's `role="row"`; Angular cannot project one set of children into several positions, so every chip is a child of the grid and each day column claims its own through `aria-owns`. The accessibility tree is the same in both layers. Two consequences are real: the day columns are CSS grid **tracks** rather than flex items, because a chip's horizontal placement is a percentage of the whole grid and unequal columns would slide the last day's chips off; and anything projected that is not an `arena-calendar-event` becomes a **grid item** and adds a column, where React silently skips it.

**The chip body is Arena's, and there is nothing you can put inside it.** An `arena-calendar-event` carries `id`, `title`, `start`, `end` and `colorId`, and Arena draws all of it. There is no per-event template — a structural directive returning markup is not a member of any Arena contract.
...
2. From an hour cell, Enter steps into an event chip; Escape steps back out. Walk
   a chip with a panel as well as one without — they are different elements.

 ...
 * needs the same two levers Table.cases.test.ts uses and undoes: happy-dom's ResizeObserver never
 * fires, and --bp-md reads empty. 768 is the only value cached here, because
 * PageHead.variants.test.ts asserts that number on a cache hit and the cache is shared. */

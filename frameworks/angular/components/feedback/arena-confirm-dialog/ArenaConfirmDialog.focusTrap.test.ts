...
import { isConfirmLocked } from './ArenaConfirmDialog';
...
test('ArenaTab from the last focusable wraps to the first, and the key is consumed', () => {
  ...
  assertSameNode(document.activeElement, input, 'ArenaTab from the last element must wrap to the first');
  assert.ok(event.defaultPrevented, 'the boundary ArenaTab must be prevented, or the browser would also move focus');
});

test('Shift+ArenaTab from the first focusable wraps to the last, and the key is consumed', () => {
  const { panel, input, confirm } = buildPanel();
  input.focus();
  assertSameNode(document.activeElement, input);
  const event = keydownTab(true);
  trapTabKey(panel, event, document.activeElement);
  assertSameNode(document.activeElement, confirm, 'Shift+ArenaTab from the first element must wrap to the last');
  assert.ok(event.defaultPrevented);
});

test('ArenaTab away from a middle element is left alone -- the trap only intervenes at a boundary', () => {
  ...
  assert.ok(!event.defaultPrevented, 'a non-boundary ArenaTab must be left to the browser\'s own default handling');
});

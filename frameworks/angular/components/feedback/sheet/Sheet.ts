...
export class Sheet {
  ...
  readonly placement = input<SheetPlacement>('bottom');
  ...
  protected readonly styles = computed(() => sheetStyles({ placement: this.placement(), open: this.open() }));

  protected readonly heading = computed(() => {
    const text = this.title();
    if (text.trim() === '') {
      throw new Error('Sheet: `title` is required, and names the panel and the control that folds it');
    }
    return text;
  });

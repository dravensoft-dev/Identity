...
| `requestChange` | event |  |  | A change was requested while `confirm` is set: the host opens a ConfirmDialog and, on confirmation, flips `state` (the requested value is always the negation of the current one). |

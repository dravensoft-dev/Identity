...
### Changed
- **The plugin is served from the release tag, not from whatever `main` happens to hold.** `marketplace.json` declared `"source": "."`, which resolves against the marketplace's own checkout — the default branch. A version string therefore did not identify a tree: 2.2.0 was declared at the `SegmentedControl` commit and `main` moved seven commits past it under the same number, so two people installing "2.2.0" a day apart could hold different code. The entry now pins `{"source": "github", "repo": "dravensoft-dev/Identity", "ref": "v2.2.0"}`, so a version resolves to exactly one commit, permanently, and the tags become the release channel rather than decoration. The catalog itself is still read from `main`, which is what lets a new release announce itself.

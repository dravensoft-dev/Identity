...
- `frameworks/react/README.md`, so all three framework layers have one.
- `bun run check:docs`, which holds the two new documentation norms: every `.md` under
  60,000 characters, and the comment rule below.

### Changed

- **`contracts/design/README.md` is now the normative design specification** — voice, type,
  color, spacing, motion, the danger convention, iconography and theming, plus the DTCG
  token type map. The root `README.md` previously held it.
- **`README.md` is Getting started and nothing more.** Its other sections moved to
  `contracts/README.md` (framing and the repository map), `contracts/design/README.md` (the
  design specification) and `frameworks/react/README.md` (the React layer).
- Every `.md` is written in the present tense and describes what Arena is, not what it was.
- **Comments are gone from hand-written sources.** `scripts/` and test files may carry one
  header of at most 10 lines; everything else carries none, with context moved into names.
  Files a script generates are outside the rule and are untouched.

### Removed

- `components-divergences.md` — absorbed whole into `DOUBTS.md` section 3. Citations that
  named one of its sections now point there; a citation naming only the file still resolves.
...
- **A `Calendar` chip draws its time label only when it fits in both axes.** Every chip
  ...
  vertical half of that decision; the width term was never there. Both thresholds, and the
  gutter width the new term reads, are now script-flagged tokens: the vertical one had been a
  bare literal that `check:dimensions` cannot see, because that gate reads governed CSS
  properties and never a comparison.

...
- **Angular layer parity — 19 new primitives, for 20 in all.** `activity-feed`,
  ...
  and `unauth-card` join `tag`, the reference shape. Each is a full
...
- **16 new component manifests** under `frameworks/tailwind/components/` — 18 in all —
...
- **21 more component manifests** — Button (completed: `secondary` and `ghost` were
  ...
  now ships 38 manifests: one for every React component except the three SVG charts and
...
- **`rounded-full` unified to `rounded-pill` across six manifests, and `bun run
  check:radius` now guards the regression.** `Avatar`, `Button`, `Switch`, `Badge`,
  `Radio` and `Skeleton` mixed Tailwind's own `calc(infinity * 1px)` radius with
  Arena's `--r-pill` (999px) — sometimes both in the same file. Every element carrying
  either is well under the ~1998px width where the two would visibly differ, so this
  is a naming fix, not a rendering change. `check:coverage` only asserted the converse
  (every token reaches a utility); the new gate — narrowly scoped to this one verified
  case, not "every utility traces to a token" in general — is the sixteenth, bringing
  `bun run check` to seventeen steps including the test suite.
- **Redundant `box-border` dropped from `Button.manifest.json` and
...
- **`ThemeToggle` is gone from every layer — breaking.** Arena keeps only `Switch` in
  the toggle family. Removed:
  `frameworks/react/components/forms/ThemeToggle.{jsx,js,d.ts,prompt.md,behaviour.json}`,
  `frameworks/angular/primitives/theme-toggle/` (whole directory, dropped from the
  primitives barrel), `frameworks/tailwind/components/ThemeToggle.{manifest.json,manifest.ts,card.html}`,
  and its demo in `forms.card.html`/`forms.card.entry.jsx`. Its two entries in
  `components-divergences.md` are deleted, because the divergences no longer exist. The
  Delivery Console's header theme switch (`Shell.jsx`) is now a `Switch` wired to
  `window.__toggleTheme` and the `arena-light` class, owning `isDark` state itself
  instead of observing the DOM through a `MutationObserver`. The Angular layer drops to
  20 primitives, and the Tailwind layer to 38 manifests (17 of which mirror an Angular
  primitive).

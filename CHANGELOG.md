...
- **Breaking for any script or command naming a path under `scripts/`: the tooling sorts itself
  ...
  and `arena` (two or more layers, or the repository root). `scripts/lib/` stays where it is and
  gains every test that covers one of its modules; only `serve.mjs` remains loose. Every
  generated file's `GENERATED by` banner now names the generator's real path, and
  `scripts/README.md` states the shape.
- **Two npm scripts are renamed, because they generate rather than compile**: `build:tokens` is
  now `generate:tokens`, and `build:api` is now `generate:api`. An npm script's prefix names its
  phase directory. The other five `build:*` scripts are unchanged.

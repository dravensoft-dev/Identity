...
### Fixed
- **`Button` and `IconButton` now read the density tokens** (`--dz-ctl-h-sm` / `--dz-ctl-h` / `--dz-ctl-h-lg`) instead of hard-coding 32/40/48. The tokens were declared for "buttons, inputs, switches row" and their values were plainly taken from Button's own scale, but nothing ever read them: `Table` was the sole consumer, so `.arena-compact` re-densified rows while every button in the same view stayed comfortable. Both components change together because they share one scale and sit side by side in toolbars. No visual change outside `.arena-compact`; inside it a medium button is now 32px rather than 40px.

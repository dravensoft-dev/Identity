...
- **`SideNav`** (navigation) — the sidebar's navigation list, and only the list. Every
  ...
  the consumer writes one `SideNavItem` per destination, optionally grouped under a
  `SideNavSection` or nested inside a `SideNavCollapsible`, and `SideNav`
  injects which id is active, the handler that reports an activation, and each child's
  nesting depth — the `Table`/`TableRow` shape one size down, with arbitrary nesting.
  An item with `href` renders an anchor; without one, a button.

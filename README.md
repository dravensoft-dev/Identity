...
- **"Danger is outline" governs controls and surfaces, not presence.** `Avatar`'s presence dot (online/busy/away/offline) is a different semantic family — a status taxonomy, like the chart `tone` colors, not a destructive affordance — and it is filled: `--color-success`, `--color-warning` and `--color-error` for the three live states, `--status-offline` for the fourth. An outline dot at that size (`max(8px, diameter * 0.28)`) would not read at all. Nothing here contradicts the rule above: the rule is about *danger*, and a busy dot filled in `--color-error` is presence borrowing the error hue for "occupied," not a risk indicator asking to be read as danger.
...
- `frameworks/react/use-container-width.js` — shared `useContainerWidth` hook and `readBreakpoint`; responsive components import it. `theme.js` — theme toggle helper. `frameworks/react/vendor/` — a committed, generated CommonJS→ESM bundle of React for the demo pages' importmap (`build-vendor.mjs`); every component `.jsx` and demo `.entry.jsx` has a compiled `.js` sibling the pages actually load (`build-demos.mjs`, `check:demos`).

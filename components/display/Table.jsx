...
export function Table({ columns = [], rows = [], getRowKey, onRowClick, empty = 'No data.', style }) {
  ...
  return (
    <div style={{ border: '1px solid var(--color-base-300)', borderRadius: 'var(--r-lg)', overflow: 'hidden', background: 'var(--surface-card)', ...style }}>
          ...
          <tr style={{ background: 'var(--panel)' }}>
            {columns.map((c) => (
              <th key={c.key} style={{ ...cellBase, textAlign: c.align || 'left', width: c.width,
                ...
                color: 'var(--mute)', fontWeight: 700, borderBottom: '1px solid var(--color-base-300)' }}>{c.header}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          ...
          {rows.map((row, ri) => (
            <tr key={getRowKey ? getRowKey(row, ri) : ri}
              ...
              style={{ borderTop: ri === 0 ? 'none' : '1px solid var(--color-base-300)', cursor: onRowClick ? 'pointer' : 'default',

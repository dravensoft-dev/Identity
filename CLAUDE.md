...
cropped to it and the overflow is lost silently. Declaring it by arithmetic does not work —
...
`Bun.Transpiler` — neither builder exists under plain `node`. Where a dependency is missing the
gate exits 2, `check-all` marks it `SKIP` and reports the whole run `INCOMPLETE` rather than
green; `ARENA_CHECK_STRICT=1` — or `CI=true`, so an automated run never skips quietly — makes

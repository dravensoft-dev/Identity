...
`bun scripts/generate/arena/generate-tokens.mjs` (`bun run generate:tokens`). **Never edit those four CSS
...
(`scripts/check/arena/check-manifest-states.mjs`) flags a `hover:`/`focus:`-family Tailwind state modifier
...
`check/` by domain and has its own README) plus `intro/`, the browsable front: `styles.css`,
...
`Bun.Transpiler` — neither builder exists under plain `node`, which leaves each with nothing
to compare against. Where a dependency is missing the gate exits 2, and

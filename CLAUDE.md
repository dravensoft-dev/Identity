...
`bun run check` runs all twelve plus the test suite, without stopping at the first failure. **Two gates are not runtime-portable**: `check:cards` needs a headless browser (`CHROME_PATH`, or Chromium on the usual paths), and `check:vendor` needs `Bun.build` to rebuild `frameworks/react/vendor/*.js` for comparison, which only exists under Bun (see `scripts/build-vendor.mjs`) — under plain `node scripts/check-all.mjs` it has nothing to compare against. Where either dependency is missing the gate exits 2, and `check-all` marks it `SKIP` and reports the whole run `INCOMPLETE` rather than green; `ARENA_CHECK_STRICT=1` — or `CI=true`, so an automated run never

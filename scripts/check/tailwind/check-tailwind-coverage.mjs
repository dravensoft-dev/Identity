...
export const EXCLUDED = new Map([
  ...
  ['press-scale', 'a transform scale rather than a length, so no @theme namespace owns it -- both layers read it as var(--press-scale), React inside its injected .arena-btn-press rule and the Button manifest as active:scale-[var(--press-scale)]. This gate checks theme keys, and a bracket referencing the custom property directly is the correct shape for a value with no namespace'],

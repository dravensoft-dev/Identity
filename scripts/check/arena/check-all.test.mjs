...
test('GATES lists the thirty check gates', () => {
  assert.equal(GATES.length, 30);
  assert.deepEqual(
    GATES.map((g) => g.name),
    ['check:docs', 'check:generated', 'check:dtcg', 'check:tokens', 'check:script-tokens', 'check:duplicate-constants', 'check:ramp', 'check:text-contrast', 'check:tailwind', 'check:tailwind-generated', 'check:coverage', 'check:surface-parity', 'check:radius', 'check:arbitrary', 'check:dimensions', 'check:states', 'check:structure', 'check:contracts', 'check:behaviour', 'check:compliance', 'check:api', 'check:fonts', 'check:vendor', 'check:demos', 'check:cards', 'check:focus-trap', 'check:angular', 'check:angular-demos', 'check:assertions', 'check:cdk'],
  );
});

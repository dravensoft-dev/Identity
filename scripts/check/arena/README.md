...
| `check-docs.mjs` | a `.md` exceeds 60,000 characters, a `.md` punctuates its prose with an em dash, or a hand-written source carries a comment it is not allowed: `scripts/` and tests may carry one header of at most 10 lines, everything else none. `SIZE_EXEMPT` names the three documents exempt from the limit by charter and `PROSE_EXEMPT` the one tree exempt from the punctuation rule, each with its reason. Both rules read by lexing, so a `//` inside a string is never mistaken for a comment and an em dash inside a fence or a code span is the document quoting code; a `.generated.` file is skipped unread. It also holds the branch boundary: a `.prompt.md` is the consumer's last stop, so citing a path under `scripts/`, a `README.md` under `contracts/` or `frameworks/`, or `frameworks/PACKAGING.md` sends a reader somewhere the router promised they would not have to go. `CONTRIBUTOR_PATHS` carries a reason per shape and `EXEMPT` does not exist, because a prompt needing one is a rule stated on the wrong branch. |

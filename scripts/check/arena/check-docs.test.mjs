...
test('a generator is not its own output: the marker counts only in the header comment', () => {

  const generator = "import { join } from 'node:path';\nexport const BANNER = '/* GENERATED by scripts/x.mjs */';\n";
  assert.equal(isGenerated('scripts/x.mjs', generator), false);

  const late = `${'// c\n'.repeat(2)}const a = 1;\n/* GENERATED by x.mjs */\n`;
  assert.equal(isGenerated('a/B.ts', late), false);

  const padded = `/* GENERATED by x.mjs\n${' *\n'.repeat(200)} */\nconst a = 1;\n`;
  assert.ok(padded.indexOf('GENERATED by') < 400 && padded.length > 400);
  assert.equal(isGenerated('a/B.ts', padded), true);

  const shifted = `${"import { a } from './a.mjs';\n".repeat(20)}/* GENERATED by x.mjs */\nconst a = 1;\n`;
  assert.ok(shifted.indexOf('GENERATED by') > 400, 'the marker must be past the old 400-character window');
  assert.equal(isGenerated('a/B.ts', shifted), false, 'past the window, and not the header either');
});

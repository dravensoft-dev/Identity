...
test('the react half asks only the components that draw by hand, because the rest answer with their manifest', () => {
  assert.deepEqual([...HAND_DRAWN.keys()].sort(), ['ArenaBarChart', 'ArenaDoughnutChart', 'ArenaHorizontalBarChart', 'ArenaLineChart', 'ArenaPyramidChart']);
  assert.deepEqual(unaskedHandDrawn(['ArenaBarChart', 'ArenaDoughnutChart', 'ArenaHorizontalBarChart', 'ArenaLineChart', 'ArenaPyramidChart']), []);
});
...
test('a hand-drawn component the react half never opened is a failure, not a clean pass', () => {
  ...
  assert.deepEqual(problems.map((p) => /HAND_DRAWN names (\w+)/.exec(p)?.[1]).sort(),
    ['ArenaHorizontalBarChart', 'ArenaLineChart', 'ArenaPyramidChart']);

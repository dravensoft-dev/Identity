...
| `check-ramp.ts` | the 8-slot categorical chart ramp stops clearing its contrast and colour-vision-deficiency gates on either surface. Run by path, with no npm script. |
| `check-text-contrast.ts` | a text token fails contrast against the surface it is declared for, or a retired token reappears in `colors.css`; `REMOVED` names each one with the token that replaces it. Run by path, with no npm script. |

...
| `check-ramp.mjs` | the 8-slot categorical chart ramp stops clearing its contrast and colour-vision-deficiency gates on either surface. Run by path — no npm script. |
| `check-text-contrast.mjs` | a text token fails contrast against the surface it is declared for, or a token removed in 2.0.0 reappears in `colors.css`. Run by path — no npm script. |

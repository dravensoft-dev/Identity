...
| `check-kit-generated.mjs` | `frameworks/react/kit/` differs from a fresh `build:kit`, in either direction: a stale file, one the layer no longer produces, or a build that wrote nothing at all. The kit is the one build product this repository tracks, because it is the payload the copy-in channel hands a consumer verbatim. Needs `Bun.Transpiler`; exits 2 under plain node. |

/* Generates contracts/design-generated/{palette,typography,spacing,effects}.css
 * from the DTCG source in contracts/design/.
 *
 ...
 *  rather than once into contracts/design/ so that a component's import never
 *  crosses the contracts/design/ <-> frameworks/ boundary: the import is intra-layer, the same shape
 ...
 *
 *  The constraint is self-imposed and removable: load all of contracts/design/
 *  into one dictionary and partition the walk by each token's source file.
 *  Nobody has needed it enough to do it. If you are here because you wanted an
 *  alias, that is the fix — do not add a fourth restatement. */

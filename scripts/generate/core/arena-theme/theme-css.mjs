 ...
 * palette blocks and the @font-face rules. Everything else about a token is already
 * decided and travels inside the package. This module runs in the repository, where
 * check-packages.mjs holds its output equivalent to the Style Dictionary pipeline, and
 * inside both npm packages, where it is the only emitter there is. It reads no file and
 * touches no network, so a path in the config is emitted, never resolved. */
...
export function themeCss(config, options = {}) {
  const { packageName = '@dravensoft/arena-react', importHeader = true, source = 'arena.config.json' } = options;
  const fallbackFor = (role) => config.fonts[role].fallback ?? FONT_ROLES[role];

  const parts = [`/* GENERATED by arena-theme from ${source}. Edit that, not this file. */`];
  if (importHeader) parts.push(`@import '${packageName}/arena.css';`);

  const roles = Object.keys(FONT_ROLES);
  const sheets = [...new Set(roles.map((r) => config.fonts[r].src).filter(isStylesheet))];
  for (const sheet of sheets) parts.push(`@import url('${sheet}');`);

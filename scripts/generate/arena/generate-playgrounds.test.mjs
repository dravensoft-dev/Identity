...
test('the pilot emits a page and an entry into both layers, and nothing else', () => {
  const files = [...componentFiles(root, ['Card']).keys()].sort();
  assert.deepEqual(files, [
    'frameworks/angular/components/display/card/Card.demo.entry.generated.ts',
    'frameworks/angular/components/display/card/Card.demo.generated.html',
    'frameworks/react/components/display/card/Card.demo.entry.generated.tsx',
    'frameworks/react/components/display/card/Card.demo.generated.html',
  ]);
});

test('a page lands beside the component it draws, so the two URLs differ in one path segment', () => {
  const files = [...componentFiles(root, ['Card']).keys()];
...
test('a component reaching something frameworks/Components.json does not name fails by name', () => {
  ...
  assert.throws(
    () => placesFor({ component: 'Card', uses: ['Nope'] }, all),
...
test('every emitted file names its generator, so nobody edits the copy', () => {
  for (const [rel, body] of componentFiles(root, ['Card'])) {

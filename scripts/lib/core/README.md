...
| `validate-palette.mjs` | Contrast and colour-vision-deficiency validation for the chart ramp. **Vendored verbatim** from the `dataviz` Agent Skill, and re-vendored rather than patched, because the thresholds and CVD matrices are calibrated to the Machado-Oliveira-Fernandes (2009) severity-1.0 model, and editing one invalidates published measurements. |

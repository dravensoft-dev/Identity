...
| `--z-sheet` | 850 | nothing Arena draws either: a non-modal panel anchored to an edge, a cart, a filter drawer, a detail pane that leaves the page usable behind it. Above nav, because a sheet that slides over a fixed bar and lands under it is the failure; below dropdown, because a `Menu` opened from inside the sheet belongs over it |
